...
public class PRQuadBlackNode extends PRQuadNode {
    ...
    private ArrayList<KDPoint> blkArray; 
    ...
    public PRQuadNode insert(KDPoint p, int k) {
        ...
        if (this.blkCount > this.bucketingParam) {
            if (k < 1) {
                throw new CentroidAccuracyException(null);
            }
            PRQuadGrayNode nw = new PRQuadGrayNode(this.centroid, k, this.bucketingParam );
            nw.gryHeight = this.height();
            for (KDPoint i: this.blkArray) {
                nw.insert(i, k);
            }
            return nw;
        }
    ...
    public void range(KDPoint anchor, Collection<KDPoint> results,
                      double range) {
        //throw new UnimplementedMethodException(); // ERASE THIS LINE AFTER YOU IMPLEMENT THIS METHOD!
        //System.out.println("Centriod: " + this.centroid);
        for (KDPoint i: this.blkArray) {
            if ( KDPoint.euclideanDistance(anchor, i) <= range) {
                if(!anchor.coords.equals(i.coords)) {
                    results.add(i);
                }
            }
        }
    ...
    public NNData<KDPoint> nearestNeighbor(KDPoint anchor, NNData<KDPoint> n) {
        //throw new UnimplementedMethodException(); // ERASE THIS LINE AFTER YOU IMPLEMENT THIS METHOD!
        for (KDPoint i: this.blkArray) {
            if (n.getBestDist() == INFTY) {
                if (!anchor.coords.equals(i.coords)){
                    n.update(i, KDPoint.euclideanDistance(anchor, i) );
                }
            } else {
                if (KDPoint.euclideanDistance(anchor, i) <= n.getBestDist()) {
                    if (!anchor.coords.equals(i.coords)){
                        n.update(i, KDPoint.euclideanDistance(anchor, i) );
                    }
                }
    ...
    public void kNearestNeighbors(int k, KDPoint anchor, BoundedPriorityQueue<KDPoint> queue){
        for (KDPoint i: this.blkArray) {
            if(!anchor.coords.equals(i.coords)) {
                queue.enqueue(i,KDPoint.euclideanDistance(anchor, i) );
            }
        }
    }
}
