...
public class StudentTests {
    ...
    @Test 
    public <T> void boundedPriorityTest1() {
        BoundedPriorityQueue<T> nw = new BoundedPriorityQueue<>(5);
        Iterator<T> it;
        nw.enqueue( (T)"1", (double) 10);
        nw.enqueue( (T)"2", (double) 4);
        nw.enqueue( (T)"3", (double) 3);
        nw.enqueue( (T)"4", (double) 2);
        nw.enqueue( (T)"5", (double) 1);

        nw.enqueue( (T)"9", (double) 5);

        System.out.println("Size: "+ nw.size());
        it = nw.iterator();
        while (it.hasNext()){
        ...
        System.out.println("[First: " + nw.first() +"] [Last: " + nw.last());
        System.out.println("Size: "+ nw.size());
    ...
    public void testPRQSimpleQuadTree(){
        ...
        prQuadTree.insert(new KDPoint(1, 7));
        prQuadTree.insert(new KDPoint(5, 1));
        prQuadTree.insert(new KDPoint(4, 2)); // Should fit
        prQuadTree.insert(new KDPoint(-1,1));
        prQuadTree.insert(new KDPoint(2,-1));
        prQuadTree.insert(new KDPoint(-1,3));
        prQuadTree.insert(new KDPoint(-1,-1));
        prQuadTree.insert(new KDPoint(-5,5));
        ...
        //    2, prQuadTree.height());
        //assertEquals("After t inserwotions into a PR-QuadTree, the count should be 2.", 7, prQuadTree.count());
/*
        ArrayList<String> PRQSimple = prQuadTree.treeDescription(false);
        CompactVizTree visualizer = new CompactVizTree(200,120,10);
        visualizer.drawBTreeToFile(PRQSimple, 4, "simplePRQTree");

        prQuadTree.delete(new KDPoint(-5,5));


        PRQSimple = prQuadTree.treeDescription(false);
        visualizer.drawBTreeToFile(PRQSimple, 4, "simplePRQTreeDelete");
        assertEquals("After two insertions into a PR-QuadTree with b = 2, the result should be a quadtree consisting of a single black node.",
        1, prQuadTree.height());
*/
    ...
    @Test
    public void testPRQSimpleQuadTreeRecursive(){
        prQuadTree = new PRQuadTree(5, 1); // Space from (-8, -8) to (8, 8), bucketing parameter = 2.
        prQuadTree.insert(new KDPoint(1, 1));
        prQuadTree.insert(new KDPoint(2, 1));
        prQuadTree.insert(new KDPoint(5, 1));
        prQuadTree.insert(new KDPoint(4, 2)); // Should fit
        ...
        CompactVizTree visualizer = new CompactVizTree(200,120,10);
        visualizer.drawBTreeToFile(PRQSimple, 4, "simplePRQTree");
        //assertEquals("Count", 2, prQuadTree.count());
        //assertEquals("Height",
        //   4, prQuadTree.height());
...
//        expectedKnnPoints.enqueue(new KDPoint(-2.0, 7.0),2.236068);
    ...
    public void testPRQRange() {
        ...
        visualizer.drawBTreeToFile(kdDescription,4,"compact_quadtree_range");
    }
    @Test
    public void testPRQRange2() {
        prQuadTree = new PRQuadTree(4, 2); // Space from (-8, -8) to (8, 8), bucketing parameter = 2.
        KDPoint[] points = {new KDPoint(1, 1),
            new KDPoint(4, 2),
            new KDPoint(7, 2),
        ...
        ArrayList<KDPoint> ptsWithinRange = new ArrayList<>(prQuadTree.range(new KDPoint(0,0),5.0));
        System.out.print(ptsWithinRange.toString());

      

        ArrayList<String> kdDescription = prQuadTree.treeDescription(false);
        CompactVizTree visualizer = new CompactVizTree(120,120,10);
        visualizer.drawBTreeToFile(kdDescription,4,"compact_quadtree_range2");
    }
    ...
    public void testKDTreeViz(){
        ...
        visualizer.drawBinaryTreeToFile(kdDescription,"compact_kdtree_before_delete");

        System.out.println("---------------------");
        kdTree.delete(new KDPoint(12, 18));
        //kdTree.insert(new KDPoint(20,600));

        kdDescription = kdTree.treeDescription(false);
        visualizer.drawBinaryTreeToFile(kdDescription,"compact_kdtree_after_delete");
    ...
    public void testPRTreeViz(){
        prQuadTree = new PRQuadTree(4, 1); // Space from (-8, -8) to (8, 8), bucketing parameter = 2.
